...
func TestParseStreamDataBlocked(t *testing.T) {
	...
	var frame StreamDataBlockedFrame
	l, err := parseStreamDataBlockedFrame(&frame, data, protocol.Version1)
...
func TestParseStreamDataBlockedErrorsOnEOFs(t *testing.T) {
	...
	require.Equal(t, len(data), l)
	for i := range data {
		_, err := parseStreamDataBlockedFrame(&frame, data[:i], protocol.Version1)
