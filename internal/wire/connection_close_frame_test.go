...
func TestParseConnectionCloseTransportError(t *testing.T) {
	...
	l, err := parseConnectionCloseFrame(&frame, data, connectionCloseFrameType, protocol.Version1)
...
func TestParseConnectionCloseWithApplicationError(t *testing.T) {
	...
	var frame ConnectionCloseFrame
	l, err := parseConnectionCloseFrame(&frame, data, applicationCloseFrameType, protocol.Version1)
	require.NoError(t, err)
...
func TestParseConnectionCloseLongReasonPhrase(t *testing.T) {
	...
	var frame ConnectionCloseFrame
	_, err := parseConnectionCloseFrame(&frame, data, connectionCloseFrameType, protocol.Version1)
...
func TestParseConnectionCloseErrorsOnEOFs(t *testing.T) {
	...
	require.NoError(t, err)
	for i := range data {
		_, err = parseConnectionCloseFrame(&frame, data[:i], connectionCloseFrameType, protocol.Version1)
		require.Equal(t, io.EOF, err)
	}
}
